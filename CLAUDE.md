...
- `providers/`: Provider definitions (`anthropic.sh`, `openai.sh`, `openrouter.sh`, `minimax.sh`, `antigravity.sh` for Gemini, `antigravity-claude.sh` for Claude, `deepseek.sh`, `ollama.sh` for local/cloud Ollama models via its native Anthropic API).
